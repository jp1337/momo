...
- **Gamification** — Earn coins, build streaks (with a monthly Streak Shield that protects against a single missed day), level up. Small wins deserve real celebration.
...
- **Push Notifications** — Daily quest reminders via browser push, plus an opt-in "due today" reminder that lists the tasks actually due today (silent on empty days — no "all clear" pings). Works via Web Push, ntfy.sh, Pushover, Telegram, or Email. **Notification History** in Settings shows the last 50 delivery attempts with status and error details for debugging.
