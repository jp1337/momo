...
- `i18n-key-families.mjs` — Das Familienregister zu `check-i18n.mjs`. Eine Familie sind Keys, die im Code nie als Literal auftauchen — per Template-Literal gebildet (``t(`catalog.${key}.title`)``) oder als Variable aus einer Aufzaehlung gereicht (`t(config.key)`, `t(LEVEL_META[level].labelKey)`) — und die ein Literal-Scan darum nicht sehen kann. `members()` MUSS aus einer codeseitigen Aufzaehlung ableiten (`ACHIEVEMENT_DEFINITIONS`, `LEVELS`, `VALID_TABS`, `TEMPLATES`, `LOCALES`, `pgEnum`s aus `lib/db/schema.ts`, Konfigtabellen wie `LEVEL_META`/`RARITY_CONFIG`/`NAV_ITEMS`), nie aus den Locale-Dateien — sonst prueft die Familie sich selbst. Eine Familie ohne echte Aufrufstelle ist keine Familie, sondern ein Fund. `__tests__/check-i18n.test.ts` sichert die Ableitung ab
