...
- **Integrationstests massiv erweitert: 29 → 144 Tests** — Fünf neue Test-Dateien decken alle bisher nicht abgedeckten Business-Logic-Funktionen ab: `postponeDailyQuest`, `forceSelectDailyQuest`, `reselectQuestForEnergy`, `pinTaskAsDailyQuest` (Daily Quest Mutations), `uncompleteTask`, `snoozeTask`, `unsnoozeTask`, `bulkUpdateTasks`, `reorderTasks`, `promoteTaskToTopic`, `breakdownTask` (Task Mutations), `computeHabitStreak` als pure-function-Tests für alle vier Wiederholungstypen (INTERVAL/WEEKDAY/MONTHLY/YEARLY) plus Pause-Ranges, die gesamte Vacation Mode Logik (`activateVacationMode`, `deactivateVacationMode`, `autoEndVacations`) sowie Gamification-Extras (`getLevelForCoins`, `getNextLevel`, `getUserStats`, `checkAndUnlockAchievements`). Außerdem wurde ein bestehender flaky Test in `select-daily-quest.test.ts` gefixt: null-getaggte Tasks matchten den Energy-Preference-Filter genauso wie explizit getaggte Tasks, was zu nicht-deterministischer Task-Auswahl führte.
