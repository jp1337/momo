...
- **Integrationstests vollständig: 29 → 437 Tests** — Die Test-Suite deckt jetzt die gesamte `lib/`-Schicht ab. Phase 1 (144 Tests): Daily Quest Mutations, Task Mutations, Habit Streaks, Vacation Mode, Gamification. Phase 2 (294 Tests): Task CRUD, Topics CRUD, Wishlist, Energy Check-ins, Sessions, API Keys, Calendar Feeds, Weekly Review, User Account Management. Phase 3 (366 Tests): statistics, habits, templates, onboarding. Phase 4 (437 Tests): vier weitere Dateien für `totp.ts` (35 Tests: Setup, Code-Verifikation, enable/disable, Backup-Code Single-Use-Garantie, signed Setup-Token-Cookie), `export.ts` (DSGVO-Datenexport, User-Isolation), `email-templates.ts` (pure HTML-Rendering, XSS-Escaping), `rate-limit.ts` (Fixed-Window, Key-Isolation, Window-Expiry). 25 Test-Dateien, vollständige `lib/`-Abdeckung.
