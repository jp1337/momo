...
- **Integrationstests vollständig: 29 → 366 Tests** — Die Test-Suite deckt jetzt die gesamte `lib/`-Schicht ab. Phase 1 (144 Tests): Daily Quest Mutations, Task Mutations, Habit Streaks, Vacation Mode, Gamification. Phase 2 (294 Tests): Task CRUD, Topics CRUD, Wishlist, Energy Check-ins, Sessions, API Keys, Calendar Feeds, Weekly Review, User Account Management. Phase 3 (366 Tests): vier weitere Dateien für `statistics.ts` (`computeStreakHistory` pure function, `getUserStatistics`, `getAdminStatistics`), `habits.ts` (`getHabitsWithHistory`, `getEarliestCompletion`, `buildYearOptions` pure, `computeHabitStreak` pure), `templates.ts` (`getTemplate` pure, `importTopicFromTemplate` mit next-intl-Mock), und `onboarding.ts` (`markOnboardingCompleted`, `isOnboardingCompleted`). 21 Test-Dateien, vollständige `lib/`-Abdeckung.
