...
- **OpenAPI: 429 bei sieben Operationen nachgetragen** — die Spec dokumentierte `TooManyRequests` bereits für elf Operationen, deren Code gar kein Limit hatte. Jetzt stimmen Spec und Code überein — `DELETE /api/tasks/{id}/snooze` eingeschlossen, dessen 429-Dokumentation bis zum obigen Handler-Fix ebenfalls ein Versprechen ohne Code war. Zusätzlich: `POST /api/onboarding/complete` bekommt sein fehlendes 403 (read-only-Key), `PATCH /api/settings/timezone`, `PATCH /api/settings/vacation-mode` und `GET /api/user/export` ihr fehlendes 429 — bei den ersten beiden hing dort bereits ein `$ref` auf eine nie existierende `RateLimited`-Komponente statt auf `TooManyRequests`, also ein zweiter, unabhängiger Grund, warum das 429 dort nie ankam.
