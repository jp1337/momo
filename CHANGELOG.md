...
- **Integrationstests vollständig: 29 → 294 Tests** — Die Test-Suite deckt jetzt die gesamte `lib/`-Schicht ab. Phase 1 (144 Tests): Daily Quest Mutations, Task Mutations, Habit Streaks, Vacation Mode, Gamification. Phase 2 (294 Tests): neun weitere Test-Dateien für Task CRUD (`getUserTasks`, `getTaskById`, `createTask`, `updateTask`, `deleteTask`), Topics CRUD, Wishlist (`markAsBought`, `unmarkAsBought`, `getBudgetSummary`), Energy Check-ins, Sessions (`extractIp`, `parseUserAgent`, `listUserSessions`, `revokeSession`), API Keys (`generateApiKey`, `resolveApiKeyUser`), Calendar Feeds (Token-Rotation, ICS-Format, RRULE), Weekly Review und User-Account-Management (`deleteUser`, `updateUserProfile`). Dazu kommen Fixture-Helpers `createTestWishlistItem` und `createTestApiKey` sowie die vollständige `resetUserData()`-Bereinigung (API Keys, Webhooks, Linking Requests). Außerdem wurde ein bestehender flaky Test in `select-daily-quest.test.ts` gefixt.
