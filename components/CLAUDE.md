...
- `ui/dialog.tsx` — Thin wrapper around `@radix-ui/react-dialog` that styles the modal surface with Momo's CSS variables. Exports `Dialog`, `DialogContent` (with `title`, `description`, `size`, `hideClose` props), `DialogTrigger`, `DialogClose`. Used by all 5 modals (TaskBreakdownModal, WishlistForm, TopicForm, TemplatePicker, plus the QuickAddModal which uses Radix primitives directly to keep its custom top-aligned position). Gives focus trap, scroll lock, Esc, ARIA for free
- `ui/confirm-button.tsx` — Inline two-step confirm (replaces window.confirm)
- **Radix UI primitives in use**: `react-dialog` (modals), `react-dropdown-menu` (UserMenu, BulkActionBar topic + priority pickers, task snooze menu), `react-popover` (IconPicker). All wrappers preserve Momo's CSS-variable styling — no Radix design tokens introduced
