...
| `due_today_reminder_enabled` | boolean | Opt-in flag for the "Due today" reminder (default false). When true, the cron dispatcher sends a ping at `due_today_reminder_time` iff the user has at least one non-completed, non-snoozed task whose `due_date` (or, for RECURRING tasks, `next_due_date`) equals today in the user's timezone. Silent on empty — no "all clear" notifications |
| `overdue_reminder_enabled` | boolean | Opt-in flag for the overdue reminder (default false). Daily notification at `overdue_reminder_time` for tasks whose `due_date` is in the past (up to 30 days back) and have not been completed, snoozed, or paused. Silent on empty. Suppressed when `morning_briefing_enabled` is true |
| `recurring_due_reminder_enabled` | boolean | Opt-in flag for per-task recurring due reminders (default false). Sends individual notifications for each RECURRING task whose `next_due_date` is today (≤3 individual, >3 bundled). Suppressed when `morning_briefing_enabled` is true. Uses `recurring_due_reminder_time` for delivery timing |
| `due_today_reminder_time` | time | Delivery time for the "Due today" reminder (24h, default `08:00`). Independent of `notification_time` |
| `recurring_due_reminder_time` | time | Delivery time for recurring-due reminders (24h, default `08:00`). Independent of `notification_time` |
| `overdue_reminder_time` | time | Delivery time for the overdue reminder (24h, default `08:00`). Independent of `notification_time` |
| `weekly_review_time` | time | Delivery time for the Sunday weekly review notification (24h, default `18:00`). Previously hardcoded |
