...
| `overdue_reminder_enabled` | boolean | Opt-in flag for the overdue reminder (default false). Daily notification at `notification_time` for tasks whose `due_date` is in the past (up to 30 days back) and have not been completed, snoozed, or paused. Silent on empty. Suppressed when `morning_briefing_enabled` is true |
