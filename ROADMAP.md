...
- **Streak Shield** — einmal pro Monat überlebt ein verpasster Tag den Streak-Reset nicht; User wird am Folgetag per Notification informiert ("Dein Schild hat dich gerettet 🛡️"). DB: neue Spalte `streak_shield_used_month` (YYYY-MM) auf `users`; `updateStreak()` prüft vor Reset ob Shield noch verfügbar ist. Kleinstaufwand, massive Motivationswirkung — ein verpasster Tag sollte nicht alles zerstören.
- **Stats-Seite ausbauen** — aktuell globale Zahlen; neu: Completion-Rate pro Topic (zeigt welche Topics regelmäßig vermieden werden), beste Wochentage für Abschlüsse, Streak-Verlauf als Sparkline, Quest-Ablehnungs-Gründe (sobald "Quest ablehnen" implementiert). Alle Daten liegen bereits vor — reine Auswertungs- und Render-Arbeit in `lib/statistics.ts` + Stats-Page.
- **Bulk-Aktionen auf Tasks** — Mehrfachauswahl per Checkbox; Aktionsleiste erscheint sobald ≥1 Task ausgewählt: Löschen, Topic wechseln, Priorität setzen, alle erledigen. Nützlich beim Aufräumen nach einem Import oder beim Triage der Inbox. Rein client-seitiger State + ein neuer `PATCH /api/tasks/bulk`-Endpoint.
...
- **Onboarding-Flow** — geführter Erststart nach erster Anmeldung: 3–4 Schritte (erstes Topic anlegen → erste Tasks → Daily Quest kennenlernen → Notification einrichten), überspringbar, einmalig. Senkt die initiale Hürde für neue Nutzer die die App-Konzepte (Quest, Energie, Coins) nicht sofort verstehen. State in `users.onboarding_completed` (boolean).
...
- **Morgen-Briefing (Daily Digest)** — opt-in tägliche Zusammenfassung statt mehrerer Einzel-Pushes: Quest des Tages + heute fällige Recurring Tasks + aktueller Streak + ggf. frisch freigeschaltetes Achievement — alles in einer Nachricht über den bevorzugten Kanal. Eigene Uhrzeit (Default: 08:00), eigener `morning-briefing`-Cron-Job. Ersetzt für viele User den einzelnen Quest-Reminder + Due-Today-Reminder. Settings-Toggle sichtbar sobald ein Kanal konfiguriert ist.
- **Wunschliste mit Coins freischalten** — Wishlist-Items bekommen ein optionales `coin_price`-Feld; der "Kaufen"-Button ist nur aktiv wenn der User genug Coins hat — beim Klick werden Coins abgezogen und das Item als BOUGHT markiert. Schließt den Gamification-Loop: Coins sind kein Selbstzweck mehr, sondern Währung für echte Wünsche. DB: neue Spalte `coin_price` (nullable integer) auf `wishlist_items`; `buyWishlistItem()` prüft Guthaben und bucht atomar in einer Transaction.
