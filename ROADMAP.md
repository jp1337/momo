...
- ✅ **Erweiterte Wiederholungsregeln (Wochentag / Monat / Jahr)** — vier Regeltypen: **INTERVAL** (rollend, bisheriges Verhalten), **WEEKDAY** (z. B. Mo + Mi), **MONTHLY** (gleicher Tag jeden Monat), **YEARLY** (gleiches Datum jedes Jahr). Fixed/Rolling-Toggle: fest = nächste Fälligkeit immer ab geplantem Datum (kalenderausgerichtet); rollend = ab Abschluss-Datum. iCal-Export generiert typgerechte RRULEs (`FREQ=WEEKLY;BYDAY=MO,WE`, `FREQ=MONTHLY;BYMONTHDAY=N`, `FREQ=YEARLY;BYMONTH=M;BYMONTHDAY=D`). Habit-Tracker zählt Streaks in der natürlichen Einheit (Wochen/Monate/Jahre). DB: neues Enum `recurrence_type`, neue Spalten `recurrence_weekdays` + `recurrence_fixed` auf `tasks` (Migration `0027`). i18n in de/en/fr.
...
- ✅ **Notification-Scheduler erweitern: konfigurierbare Uhrzeit pro Reminder-Typ** — jeder opt-in Reminder (Fällig-heute, Wiederkehrende fällig, Überfällig, Wochenrückblick) hat jetzt eine eigene Uhrzeit-Einstellung, unabhängig von `notificationTime`. Time-Picker erscheint unter dem Toggle (nur wenn aktiviert). `weeklyReviewTime` (Standard `18:00`) löst die zuvor hartcodierte Sonntag-18:00-Uhrzeit ab. DB: 4 neue `time`-Spalten auf `users` (Migration `0030_broken_xorn.sql`). `PATCH /api/push/subscribe` akzeptiert alle 4 neuen Felder. — **Benutzerdefinierte Jobs** bleiben als Folge-Ticket offen.
