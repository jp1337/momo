...
api/
  ...
  tasks/[id]/promote-to-topic/route.ts → POST (convert a standalone task into the first task of a brand-new topic; returns {topicId})
  ...
  user/api-keys/route.ts             → GET (list API keys), POST (create key — returns plaintext key once)
  user/api-keys/[id]/route.ts        → DELETE (revoke key by ID)
  ...
  settings/login-notification/route.ts → GET (login-notification settings), PATCH (toggle new-device login alert; body: {enabled: boolean})
